...
def upload_file():
    """ Taking two images file and a morphing rate
        as input through the POST request in index.html
        and uploading the images to serve.
        Then pass the url of two images and morphing rate
        to makeMorph function in faceMorph.py to produce
        a morphed image.
        If morph successes, return url of morphed result and
    ...
    if request.method == 'POST':
        ...
        if file1 and allowed_file(file1.filename) and file2 and allowed_file(file2.filename):
            ...
            morph_result = faceMorph.makeMorph(save_to1, save_to2, morph_rate)
            if (type(morph_result) != str):
                flash("The model cannot learn one of the images points. Make sure you upload\
                a clear human face like the examples in about page")
